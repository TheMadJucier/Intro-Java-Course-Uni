...
public class LetterSquare {
    ...
    private boolean isValid(String letter, int wordNum, int charNum) {        
        ...
        //first of first word
        //any letter first word
        //first letter second word
        //last letter second word with repeat

        //Valid Word
        //Check if the next letter is on the same side
        //System.out.println("Hello");
        //System.out.println("words[0].charAt(wordNum-1): "+words[0].charAt(charNum-1));
        ...
        if(charNum == 0){
            ...
            String lastChar = lastLetter(word);
            if(lastChar.equals(letter)){
                return true;
            }
            return false;
        }
        String word2 = words[wordNum];
        String prevChar = word2.substring(charNum-1, charNum);
        if(onSameSide(prevChar, letter)){
            return false;
        }
        if(alreadyUsed(word2 + letter)){
            return false;
        }
        if(dictionary.hasString(word2 + letter)){
            return true;
        }
        return false;
        ...
        //check if the next letter is a prefix or a word
        //check if the next letter is the first letter
        //and if so check to see if that letter is the last letter of the solution before it
    ...
    private boolean solveRB(int wordNum, int charNum, int maxWords) {
        /* Replace this line with your implementation of the method. */
       //s System.out.println("letter.toString(): " + Arrays.toString(letters));
            
            if(dictionary.hasFullWord(words[wordNum]) && words[wordNum].length() >= 3 && allLettersUsed()){
            ...
            for(int i = 0; i < this.letters.length; i++){ 
                if(this.isValid(this.letters[i], wordNum, charNum)){
                    this.addLetter(letters[i], wordNum);
                    if(this.solveRB(wordNum, charNum+1, maxWords)){ // expand charNum
                        return true;
                    } else if(dictionary.hasFullWord(words[wordNum]) && words[wordNum].length() >= 3){ // expand charNum
                        if( this.solveRB(wordNum + 1, 0, maxWords)){ //expand wordNum
    ...
    public static void main(String[] args) {
        ...
        // should get false
       // System.out.println(test.isValid("a", 0, 1));
        ...
        // String[] sides = new String[]{"puv", "rce", "otl", "diy"};
