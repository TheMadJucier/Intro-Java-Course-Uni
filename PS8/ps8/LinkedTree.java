...
public class LinkedTree {
    ...
    private class PostorderIterator implements LinkedTreeIterator {
        ...
        private PostorderIterator() {
            Node trav = root; // initialize traversal node at root
            while (trav.left != null || trav.right != null) {
                while (trav.left != null) { //traverse down left
                    trav = trav.left;
                }
                // no left child? go right (if there) and repeat loop
                if (trav.right != null && trav.left == null) {
                    trav = trav.right;
                }
            }
            nextNode = trav; //nextNode initialized to trav
        }
        ...
        public int next() {
            ...
            if (nextNode.parent != null) {
                if (nextNode == nextNode.parent.right) { //check if right child of parent
                    nextNode = nextNode.parent; // assign nextnode to parent
                } else if (nextNode.parent.right != null) { // if parent has right child
                    Node trav = nextNode.parent.right; // initialize to right child of parent
                    while (trav.left != null || trav.right != null) {
                        while (trav.left != null) { //traverse down left
                            trav = trav.left;
                        }
                    ...
                    nextNode = trav;
                } else { // if no right child, go to parent
                    nextNode = nextNode.parent;
                }
            } else {
                nextNode = null; // if no parent, it's root we done
            }
    ...
    public static void main(String[] args) {
        ...
        int[] arr = {10, 5, 15, 2, 1, 7, 12, 17};
        tree.insertKeys(arr);
        System.out.println("tree constructed by calling insertKeys int array {10, 5, 15, 2, 1, 7, 12, 17}");
        System.out.println("expected order: 1 2 7 5 12 17 15 10");
        ...
        System.out.println();
        System.out.println("Passed test: " + s.equals("1 2 7 5 12 17 15 10 "));
    }
}
