...
public class LLList implements List {
    ...
    void rotate(int k){
        //Make a new LLList
        //Set refernce to New LLList rather than the passed List
        ...
        //LLList item = new LLList();
        Node rear = new Node(null,null);  // New Rear

        LLList rotation = new LLList();
        for(int i = 0; i < this.length; i++){ //finding the rear
            if((i+k) % this.length == this.length-1){
                rear = new Node(getNode(i).item, null);// technically this is only two calls :)
                rotation.head = rear;
            }   
        }
        ...
        int j = this.length()-1;//will decrease
        while(j >=0){
            if(rotation.length == this.length){
                break;
            }
            for(int i = 0; i < this.length; i++){ //performs rotation and searches for certain right letter
                if((i+k) % this.length == this.length-1 && j == this.length-1){ //skipping over rear node, j = 5
                    j--;// will decrease
                    break;
                } 
                if( (i+k) % this.length == j){ //right node has been found
                    Node newNode = new Node(this.getNode((i)).item,null);
                    newNode.next = rear;
                    rear = newNode;
                    rotation.head = rear;
                    j--;
